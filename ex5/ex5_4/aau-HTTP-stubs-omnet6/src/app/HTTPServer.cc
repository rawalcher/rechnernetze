...
void HTTPServer::handleMessage(cMessage *msg)
{
    ...
    if(std::string(httpMsg->getMethod())!="GET"){
        ...
        TCPControlInfo *controlInfoIN = check_and_cast<TCPControlInfo *>(httpMsg->getControlInfo());
        TCPControlInfo *controlInfo = new TCPControlInfo();
                            ...
                            controlInfo->setTcpCommand(0); // normally HTTP 0.9 closes connection immediately
                            controlInfo->setTcpStatus(controlInfoIN->getTcpStatus());
                            response->setControlInfo(controlInfo);
